...
const EORZEA_HOUR = 175 * 1000; // エオルゼア時間での1時間を現実時間でのミリ秒に変換
const EORZEA_8_HOUR = 8 * EORZEA_HOUR; // エオルゼア時間での8時間を現実時間でのミリ秒に変換
const EORZEA_DAY = 24 * EORZEA_HOUR; // エオルゼア時間での1日を現実時間でのミリ秒に変換
...
export const getWeather = (zone: string, time: number): WeatherForecast => {
...
const calculateWeather = (time: number) => {
    const eorzeaHoursFromEpoch = time / EORZEA_HOUR;
    const eorzeaDaysFromEpoch = time / EORZEA_DAY;

    const temp =
        ((eorzeaDaysFromEpoch << 32) >>> 0) * 100 +
        ((eorzeaHoursFromEpoch + 8 - (eorzeaHoursFromEpoch % 8)) % 24);
    const temp2 = ((temp << 11) ^ temp) >>> 0;
    const ret = (((temp2 >>> 8) ^ temp2) >>> 0) % 100;
    return ret;
};
